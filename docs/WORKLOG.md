...
- Local beta smoke test retry: 실패 - 리더 방 생성, 초대코드 확인, 멤버 초대코드 참여, 멤버 체크인 저장까지 통과했지만 Care Thread 메시지 전송에서 `care-message-save` 실패; Supabase 로그는 `public.care_messages` 테이블이 schema cache에 없다고 기록.
