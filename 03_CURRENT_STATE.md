...
- `MAC_MINI_HANDOFF.md`가 프로젝트 루트에 있는데 2026-06-16 시점
  내용 그대로이고 Care Thread 이후 작업이 전혀 반영 안 됨. 2026-06-29
  정리 세션에서 `.env.local`의 Supabase project ref와 이 파일의 ref가
  둘 다 `vguvxpvysodugauaxfxz`임을 확인했으므로 archive 이동 가능.
  환경변수 키/전체 URL 값은 출력하지 않음.
